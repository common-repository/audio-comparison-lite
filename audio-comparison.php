...
if (!function_exists('AC_ACL_write_log')) {
  function AC_ACL_write_log($comp, $log) {
    if (true === WP_DEBUG) {
...
if( ! class_exists( 'audioComparisonLite' ) ) {
  class audioComparisonLite {
    ...
    const MY_VERSION_NUMBER = '3.1';
    private $Styling;
    private $Promoter;
    private function write_log($log) { AC_ACL_write_log(self::SETTINGS_TITLE, $log); }
    ...
    public function enqueue_scripts() {
      ...
      wp_enqueue_script('howler-js', plugins_url('lib/howler.core.min.js', __FILE__), self::MY_VERSION_NUMBER);
      wp_enqueue_script('audio-comparison-lite', plugins_url('audio-comparison.js', __FILE__), array('jquery', 'howler-js'), self::MY_VERSION_NUMBER);
    }
    ...
    public function shortcode($attr = [], $content = null, $tag = '') {
      ...
      if ($opts['text_ready'] === "debug")
      {
        $buff = '<h3>Audio Comparison is in debug mode!</h3>';
        $buff .= '<h4>plugin_data</h4>';
        $buff .= '<p><pre>' . print_r($plugin_data, true) . '</pre></p>';
        $buff .= '<h4>opts</h4>';
        $buff .= '<p><pre>' . print_r($opts, true) . '</pre></p>';
      }
      $attr = array_change_key_case( (array) $attr, CASE_LOWER );
      if ($opts['text_ready'] === "debug")
      {
        $buff .= '<h4>lowercase attr</h4>';
        $buff .= '<p><pre>' . print_r($attr, true) . '</pre></p>';
      }
      $attr = shortcode_atts(
        array(
      ...
      $playButton = esc_html($attr['play_button']);
        ...
        $textPlay = $attr['text_button_play'];
        $textStop = $attr['text_button_stop'];
      ...
      if(!empty($fileA)) $buff .= '<a class="' . self::MAIN_DIV . '-file-a" href="' . $fileA . '"></a>' . $endl;
      if(!empty($fileB)) $buff .= '<a class="' . self::MAIN_DIV . '-file-b" href="' . $fileB . '"></a>' . $endl;
      if($playButton == "left")
      {
        $buff .= $playbuttonbuff;
        $playbuttonbuff = '';
      }
      $buttonA = $attr['text_button_a'];
      $buff .= '<button class="' . self::MAIN_DIV . '-play-a">' . html_entity_decode($buttonA) . '</button>' . $endl;
      if(!empty($fileB)) {
        $buttonB = $attr['text_button_b'];
    ...
    public function create_menu() {
      add_options_page(self::SETTINGS_TITLE, self::SETTINGS_TITLE, 'manage_options', __FILE__, array($this, 'settings_page' ) );
    ...
    public function load_settings_assets() {
      ...
      wp_enqueue_script('audiocomparisonlite_styler_js', plugins_url("styling.js", __FILE__), array('jquery'), self::MY_VERSION_NUMBER);
    ...
    public function settings_textinput($for, $disabled = false) { ?>
      <input maxlength="60" id="<?=$for?>" name="<?=$for?>" autocomplete="off" value="<?php echo esc_attr($this->get_options()[$for]); ?>"<?php echo $disabled?" disabled":"" ?> />
    <?php }
    public function settings_page() {
      ...
      if (!empty($_POST)) 
      {
        ...
        if ($current_options['text_ready'] === "debug") {
          print_r($_REQ);
        }
    ...
    ?>
    <div class="wrap <?php echo self::SHORTCODE; ?>_container">
      <style>
      .<?php echo self::SHORTCODE; ?>_settings th {
        text-align: right;
      }
      .<?php echo self::SHORTCODE; ?>_settings select {
        width: 28ch;
      }
      .<?php echo self::SHORTCODE; ?>_settings input[type=submit], input[type=color], input[type=range] {
        width: 20ch;
      }
      .<?php echo self::SHORTCODE; ?>_settings img {
        width: 200px;
      }
      .AC_ROW { display: flex; }
      ...
      <div class="AC_ROW">
        <div style="flex: 0 0 110px;">
          <img src="<?php echo plugins_url('/logo.png', __FILE__); ?>" style="width: 100px">
        </div>  
        <div style="flex: 1;">
          <p><span style="font-size: x-large"><?php echo self::SETTINGS_TITLE; ?></span> <span style="font-size: small"> Version 3.1</span></p>
          <p>
            Time synchronized A/B comparison <?php echo self::LITE; ?>of two audio files side by side. In style.<br>
            ...
            &bull; <a href="mailto:support@kaedinger.de?subject=Audio%20Comparison%20Lite%20on%20<?php echo $_SERVER['SERVER_NAME'] ?>">Questions?</a>
          </p>  
        </div>  
      ...
      <form method="post">
        <div class="AC_ROW">
          <div class="AC_COL50">
            <h2>Settings</h2>
            <table class="<?php echo self::SHORTCODE; ?>_settings">
              <tr>
                <th scope="row"><label for="text">Output:</label></th>
                <td><select id="text" name="text">
                  <option value="before"<?php echo wp_kses_post( $current_options['text'] == "before" ? " selected" : "" ); ?>>Before the buttons</option>
                  <option value="after"<?php echo wp_kses_post( $current_options['text'] == "after" ? " selected" : "" ); ?>>After the buttons</option>
          ...
          <div class="AC_COL50">
            ...
            <table class="<?php echo self::SHORTCODE; ?>_settings">
                ...
                <td colspan="4">
                  <select id="style_theme" name="style_theme">
                    ...
                    foreach (['ac','sk','ss','ep'] as $t) {
                      ...
                      $p = $d['Pro'];
                      if ($p == '1') $n = "* $n";
                      echo "<option value=\"$t\" " . ( $current_options['style_theme'] == $t ? " selected" : "" ) . ">$n</option>";
                    }
                    ?>
                  </select>
              ...
              <tr>
                ...
                <td><input type="color" id="style_color_a" name="style_color_a" value="<?php echo esc_attr($current_options['style_color_a']); ?>"></td>
                <th scope="row"><label for="style_font_size">Font size:</label></th>
                <td><input type="range" id="style_font_size" name="style_font_size" min="10" max="40" value="<?php echo esc_attr($current_options['style_font_size']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_color_b">Color B:</label></th>
                <td><input type="color" id="style_color_b" name="style_color_b" value="<?php echo esc_attr($current_options['style_color_b']); ?>"></td>
                <th scope="row"><label for="style_label_color">Label color:</label></th>
                <td><input type="color" id="style_label_color" name="style_label_color" value="<?php echo esc_attr($current_options['style_label_color']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_color_c">Color C:</label></th>
                <td><input type="color" id="style_color_c" name="style_color_c" value="<?php echo esc_attr($current_options['style_color_c']); ?>"></td>
                <th scope="row"><label for="style_label_width">Label width:</label></th>
                <td><input type="range" id="style_label_width" name="style_label_width" min="30" max="300" value="<?php echo esc_attr($current_options['style_label_width']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_width">Width:</label></th>
                <td><input type="range" id="style_width" name="style_width" min="50" max="250" value="<?php echo esc_attr($current_options['style_width']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_height">Height:</label></th>
                <td><input type="range" id="style_height" name="style_height" min="5" max="100" value="<?php echo esc_attr($current_options['style_height']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_border">Border:</label></th>
                <td><input type="range" id="style_border" name="style_border" min="0" max="20" value="<?php echo esc_attr($current_options['style_border']); ?>"></td>
              </tr>
              <tr>
                <th scope="row"><label for="style_corner">Corner:</label></th>
                <td><input type="range" id="style_corner" name="style_corner" min="0" max="50" value="<?php echo esc_attr($current_options['style_corner']); ?>"></td>
              </tr>
              <tr>
                <td colspan="2">
                  <input type="checkbox" id="apply_style" name="apply_style"<?php echo $current_options['apply_style'] == "on" ? " checked" : ""; ?>><label for="apply_style" style="font-weight: bold"> Apply style on whole site automatically</label><br>
                </td>
                <td colspan="2">
                  <button type="button" id="copy_to_clipboard" class="<?php echo self::SHORTCODE; ?>_settings button-primary">Copy style to clipboard</button>
                </td>
              </tr>
            </table>
            <p class="<?php echo self::SHORTCODE; ?>_settings">
              <input type="submit" name="save_settings" value="Save Changes" class="button-primary" />
            </p>
            <div class="tryoutarea <?php echo self::SHORTCODE; ?>_settings">
              <p>
                <div class="<?php echo self::MAIN_DIV; ?>">
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--before <?php echo self::MAIN_DIV; ?>-label-ready">Ready!</span>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--left" type="button">Play/Stop</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-a" type="button">Version A</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-b" type="button">Version B</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--right" type="button">Play/Stop</button>
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--after <?php echo self::MAIN_DIV; ?>-label-ready">Ready!</span>
                </div>
              <p>
                <div class="<?php echo self::MAIN_DIV; ?>">
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--before <?php echo self::MAIN_DIV; ?>-output-buffering">Buffering...</span>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--left <?php echo self::MAIN_DIV; ?>-button-buffering" disabled="disabled">Play/Stop</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-a <?php echo self::MAIN_DIV; ?>-button-buffering" disabled="disabled">Version A</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-b <?php echo self::MAIN_DIV; ?>-button-buffering" disabled="disabled">Version B</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--right <?php echo self::MAIN_DIV; ?>-button-buffering" disabled="disabled">Play/Stop</button>
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--after <?php echo self::MAIN_DIV; ?>-output-buffering">Buffering...</span>
                </div>
              <p>
                <div class="<?php echo self::MAIN_DIV; ?>">
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--before <?php echo self::MAIN_DIV; ?>-output-playing-a">Playing A</span>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--left <?php echo self::MAIN_DIV; ?>-playing" type="button">Play/Stop</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-a <?php echo self::MAIN_DIV; ?>-button-playing" disabled="disabled">Version A</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-b" type="button">Version B</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--right <?php echo self::MAIN_DIV; ?>-playing" type="button">Play/Stop</button>
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--after <?php echo self::MAIN_DIV; ?>-output-playing-a">Playing A</span>
                </div>
              <p>
                <div class="<?php echo self::MAIN_DIV; ?>">
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--before <?php echo self::MAIN_DIV; ?>-output-playing-b">Playing B</span>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--left <?php echo self::MAIN_DIV; ?>-playing" type="button">Play/Stop</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-a" type="button">Version A</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-b <?php echo self::MAIN_DIV; ?>-button-playing" disabled="disabled">Version B</button>
                  <button class="<?php echo self::MAIN_DIV; ?>-play-stop <?php echo self::MAIN_DIV; ?>-play-stop--right <?php echo self::MAIN_DIV; ?>-playing" type="button">Play/Stop</button>
                  <span class="<?php echo self::MAIN_DIV; ?>-label <?php echo self::MAIN_DIV; ?>-label--after <?php echo self::MAIN_DIV; ?>-output-playing-b">Playing B</span>
                </div>
                <p>
            </div>
...
<div class="doc_hide">
      <h2>Shortcode</h2>
      <p>The base version of <?php echo self::SETTINGS_TITLE; ?>'s shortcode is <code>[<?php echo self::SHORTCODE; ?> file_a="PATH/TO/AUDIO.A" file_b="PATH/TO/AUDIO.B"]</code><br>
      This way, all settings from above are used. It's a great way to keep a consistent look throughout your website for all instances of <?php echo self::SETTINGS_TITLE; ?>.<br>
      ...
      <table>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?></code></td>
          <td>The whole instance <code>div</code>.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-play-stop</code></td>
          <td>Play/Stop button.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-play-a</code></td>
          <td>Button A.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-play-b</code></td>
          <td>Button B.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-label</code></td>
          <td>Output label.</td>
        </tr>
      </table>
      <h4>Dynamic</h4>
      <table>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-output-buffering</code></td>
          <td>Assigned to the output label while audio is still loading.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-output-playing-a</code></td>
          <td>Assigned to the output label while audio A is playing.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-output-playing-b</code></td>
          <td>Assigned to the output label while audio B is playing.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-button-buffering</code></td>
          <td>Assigned to all buttons during audio loading.</td>
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-button-playing</code></td>
          <td>Assigned to the A or B button while the respective audio is playing.</td> 
        </tr>
        <tr>
          <td><code>.<?php echo self::MAIN_DIV; ?>-playing</code></td>
